if ! type git &> /dev/null; then
  ...
  function compose_git_prompt_string() {
    GIT_PROMPT_BRANCH="%F{white}on%f %F{black}$GIT_BRANCH%f"
    GIT_PROMPT_AHEAD="%F{green}↑$GIT_AHEAD%f"
    GIT_PROMPT_BEHIND="%F{blue}↓$GIT_BEHIND%f"

    GIT_PROMPT_ADDED="%F{green}+$GIT_ADDED%f"
    GIT_PROMPT_MODIFIED="%F{blue}●$GIT_MODIFIED%f"
    GIT_PROMPT_DELETED="%F{red}-$GIT_DELETED%f"
    GIT_PROMPT_UNTRACKED="%F{red}?$GIT_UNTRACKED%f"
